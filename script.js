...
var darkBgSelectors = ['.hero', '.section-dark', '.cta-band', '.marquee-strip', '.footer', '.lightbox'];
...
if (isPointer && cursorRing) {
  ...
  var hoverSel = 'a, button, .project-card, .service-card, .skill-pill, .testimonial-card, .contact-item, .ab-stage, .auto-tool, .af-step, .ab-chip';
...
───────────────────────────────────────── */
if (isPointer) {
  document.querySelectorAll('.service-card:not(.svc-featured)').forEach(function(card) {
...
/* ─────────────────────────────────────────
   AUTOMATION BUILD GALLERIES + LIGHTBOX
───────────────────────────────────────── */
...
var lbShots = [];   // <img> list of the gallery currently open
var lbIndex = 0;

function lbRender() {
  if (!lbShots.length || !lbImg) return;
  var shot = lbShots[lbIndex];
  lbImg.src = shot.getAttribute('src');
  lbImg.alt = shot.getAttribute('alt') || '';
  if (lbCaption) lbCaption.innerHTML = shot.getAttribute('data-caption') || '';
}

function lbOpen(shots, index) {
...
function lbStep(dir) {
  ...
  lbIndex = (lbIndex + dir + lbShots.length) % lbShots.length;
  lbRender();
}

if (lbClose)   lbClose.addEventListener('click', lbCloseFn);
if (lbPrevBtn) lbPrevBtn.addEventListener('click', function() { lbStep(-1); });
if (lbNextBtn) lbNextBtn.addEventListener('click', function() { lbStep(1); });
if (lightbox) {
  lightbox.addEventListener('click', function(e) {
    if (e.target === lightbox || e.target.classList.contains('lb-figure')) lbCloseFn();
  });
}
...
document.querySelectorAll('[data-gallery]').forEach(function(gal) {
  var shots  = Array.prototype.slice.call(gal.querySelectorAll('.ab-shot'));
  var thumbs = Array.prototype.slice.call(gal.querySelectorAll('.ab-thumb'));
  if (!shots.length) return;

  var captionEl = gal.querySelector('.ab-caption');
  var curEl     = gal.querySelector('.ab-cur');
  var stage     = gal.querySelector('.ab-stage');
  var index     = 0;

  function show(i) {
    index = (i + shots.length) % shots.length;
    shots.forEach(function(s, n) { s.classList.toggle('is-active', n === index); });
    thumbs.forEach(function(t, n) {
      t.classList.toggle('is-active', n === index);
      t.setAttribute('aria-selected', n === index ? 'true' : 'false');
    });
    if (captionEl) captionEl.innerHTML = shots[index].getAttribute('data-caption') || '';
  ...
  var prevBtn = gal.querySelector('.ab-prev');
  var nextBtn = gal.querySelector('.ab-next');
  ...
  var zoomBtn = gal.querySelector('.ab-zoom');
  if (zoomBtn) zoomBtn.addEventListener('click', function(e) { e.stopPropagation(); lbOpen(shots, index); });

  var frame = gal.querySelector('.ab-frame');
  if (frame) frame.addEventListener('click', function() { lbOpen(shots, index); });
